/*
Sw1: Change game speed, Hold to switch mode
Sw2: Hit bottom -> top, Hit left -> right
Sw3: Hit top -> bottom, Hit right -> left
*/
...
int task1_game_speed[3] = {10, 6, 3}; // Different game speed: 1s, 600 ms, 300 ms
...
int task1_sw1_pressed;

int task1_timer_a0_interrupt_cnt;
int task1_timer_a1_interrupt_cnt;
...
void task1_initialize(void)
{
	...
	task1_timer_a0_interrupt_cnt = 0;
	task1_timer_a1_interrupt_cnt = 0;
...
void task1_timer_a0_interrupt(void)
{
	if (++task1_timer_a0_interrupt_cnt >= task1_game_speed[task1_game_speed_choice]){
		task1_timer_a0_interrupt_cnt = 0;
		
		// Press sw1 to change game mode
		if (p8_2 == 0)
		{
			if (++task1_sw1_pressed == (30 / task1_game_speed[task1_game_speed_choice])) // 3000ms / 100ms / task1_game_speed[task1_game_speed_choice]
			{
				// Stop game
				task1_game_state = STOP;
				// Clear screen
			...
			task1_sw1_pressed = 0; // sw1 release
...
void task1_timer_a1_interrupt(void)
{
	if (++task1_timer_a1_interrupt_cnt > 9) // 100ms
	{
		task1_timer_a1_interrupt_cnt = 0;
...
void task1_sw2(void)
{
	...
	{		
		// In case restart game - new feature
		task1_lose_blinking = 0;
