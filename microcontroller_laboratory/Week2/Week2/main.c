...
int sineData[RESOLUTION] = {128,168,203,230,250,255,250,230,203,168,128,88,53,25,5,0,5,25,53,88};
int triangleData[RESOLUTION] = {51,61,71,82,92,102,92,82,71,61,51,41,31,20,10,0,10,20,31,41};
int realData[RESOLUTION] = {0};

/* min_freq = freq_range[freq_choice], max_freq = min_freq x 10 */
int freq_range[3] = {10,100,1000};
int freq_choice = 0;
float freq; 

float ampl = 2.5; // 0 to 2.5 . Not work at the moment
float percent = 0; // 0 to 100
float voltage = 0; // 0 to 5
float old_ratio = -1; // Compare to new_ratio, if difference update display (to reduce delay of display)
int data_pos = -1;

int max_adc_mode = 1023; // 1023 or 255 - 10 bit or 8 bit, depends on ADC
...
void timer_a0_interrupt(void)
{
	...
	if (++timer_a0_interrupt_cnt > 10) // 10 x 10ms
	{
		...
		ratio = (float)knob_val / max_adc_mode;
...
{	
	...
	if (mode == SINE_WAVE || mode == TRIANGLE_WAVE || mode == PLAYBACK) // DAC stuff
	{
		...
		if (mode == SINE_WAVE)
		{
			data = sineData[data_pos];			
		}
		else if (mode == TRIANGLE_WAVE)
		{
			data = triangleData[data_pos];
		}
		else
		{
...
void adc_configuration()
{
	...
	else
	{
		...
		adcon0 = 0x89; // Repeat repeat mode0, SW trigger
		if (max_adc_mode == 255)
		{
			adcon1 = 0x20;
		}
